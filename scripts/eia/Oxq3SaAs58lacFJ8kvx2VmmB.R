...
"
  ...
  ff <- carobiner::get_data(uri = uri, path = path, group = group, files = list.files("/home/jovyan/carob-eia/data/raw/eia/Ethiopia-DigitalGreen-AddOn/YieldGap/", full.names = T))
  
  # Retrieve relevant file
  f <- ff[grep("yield_gap_only_digital_green_ethiopia_06_10_2023.xlsx", basename(ff))]
  ...
  OM_amount$OM_amount <- as.integer(OM_amount$OM_amount)
  # EGB:
